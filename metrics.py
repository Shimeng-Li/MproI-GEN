...
from utils import get_mol, logP, SA, QED, weight, fingerprints, average_agg_tanimoto, compute_fragments, compute_scaffolds, get_statistics, mol_passes_filters, get_n_rings
from utils import get_dataset, disable_rdkit_log, canonic_smiles, enable_rdkit_log
from numpy import *

def get_all_descriptions(gen, n_jobs=1, device='cpu', batch_size=512, pool=None):
    """
    Computes all compounds descriptions in generated sets.
    Parameters:
    ...
    kwargs = {'n_jobs': pool}
    # kwargs = {'n_jobs': pool, 'device': device}
    # kwargs = {'n_jobs': pool, 'device': device, 'batch_size': batch_size}
    # kwargs_fcd = {'n_jobs': n_jobs, 'device': device, 'batch_size': batch_size}

    # Properties
    for name, func in [('logP', logP), ('RingNumbers', get_n_rings), ('MW', weight)]:
        descriptions[name] = WassersteinMetric(func, **kwargs).precalc(mols)
        # descriptions[name] =  mean(WassersteinMetric(func, **kwargs).precalc(mols))
...
def compute_intermediate_statistics(smiles, n_jobs=1, device='cpu', batch_size=512, pool=None) -> object:
    """
    The function precomputes statistics such as mean and variance for FCD, etc.
    It is useful to compute the statistics for test and scaffold test sets to speedup metrics calculation.
    """
    close_pool = False
    if pool is None:
        if n_jobs != 1:
    ...
    for name, func in [('logP', logP), ('SA', SA), ('QED', QED), ('MW', weight)]:
...
def cos_similarity(ref_counts, gen_counts):
    """
    Computes cosine similarity between dictionaries of form {name: count}.
    Non-present elements are considered zero: sim = <r, g> / ||r|| / ||g||
    """
...
def fraction_unique(gen, k=None, n_jobs=1, check_validity=True):
    ...
    if k is not None:
        if len(gen) < k:
            warnings.warn("Can't compute unique@{}.".format(k) +"gen contains only {} molecules".format(len(gen)))
        gen = gen[:k]
...
def get_all_metrics(gen, k=None, n_jobs=1, device='cpu', batch_size=512, pool=None, test=None, test_scaffolds=None, ptest=None, ptest_scaffolds=None, train=None):
    """
