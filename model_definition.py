...
class CharRNN(nn.Module):
    ...
    def __init__(self, vocabulary, config):
        ...
        self.vocab_size = self.input_9size = self.output_size = len(vocabulary)
    ...
    def forward(self, x, lengths, hiddens=None):
        ...
        lengths = lengths.cpu()
        x = rnn_utils.pack_padded_sequence(x, lengths, batch_first=True)  # 删除占位符
    ...
    def sample(self, n_batch, max_length=100):
        with torch.no_grad():
            starts = [torch.tensor([self.vocabulary.bos], dtype=torch.long, device=self.device) for _ in range(n_batch)]
            starts = torch.tensor(starts, dtype=torch.long, device=self.device).unsqueeze(1)
            new_smiles_list = [
                torch.tensor(self.vocabulary.pad, dtype=torch.long, device=self.device).repeat(max_length + 2) for _ in
                range(n_batch)]
